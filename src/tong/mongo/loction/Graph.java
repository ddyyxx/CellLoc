...
public class Graph {//
	...
	public double Solve(MapLoc mymp, long S, long T) {// 求S到T的最短路 避免初始化 (SPFA,可以改成dijkstra，懒得改了)
		if (S == T)
			return 0.0;
	...
	public void GetOrbit(long S, long T, Vector<Long> orbit) {// 得到从点S到点T的最短路径（弧集合）
		orbit.clear();
		long now = T;
		while (now != S) {
	...
	public double GetOrbitAtoB(MapLoc mymp, Point a, Line La, Point b, Line Lb,
			Vector<Long> orbit) {// 计算弧La到弧Lb之间的最短路（弧集合）
	...
	public void GetPobitAtoB(MapLoc mymp, Long S, Long T, Vector<Node> poset) {// 得到从一条弧到另一条弧之间的路径的点集合
		...
		double disbitStoT = GetOrbitAtoB(mymp, from.p[0], from, to.p[0], to,
				orbit);// 得到从弧S到弧T的最短路径
		int size = orbit.size();
		for (int i = 0; i < size; i++) {
