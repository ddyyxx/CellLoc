...
//			if(Algorithm.Fabs(Ta1-Ta2)>50.0){//表示这是一个Ta变化点
...
////						+" predis = "+Algorithm.Distance(car.getAnchorPoint(id).getPoint(0), car.getGpsPoint(id))
////						+" nextdis = "+Algorithm.Distance(car.getAnchorPoint(id+1).getPoint(0), car.getGpsPoint(id+1))+" time = "+car.getTime(id+1));
//				
//				total++;
//				//id++;
...
//			}
	...
	public Car readFileSolution(String filename,Map<String , double[]> map_lteloc,boolean trueTa) throws IOException, SQLException, ParseException{
		...
		while((line_poi = br_poi.readLine())!=null){//得到车辆所有点
			...
			if(Math.abs(Algorithm.Distance( lat_a,lng_a, lat_b, lng_b)-Double.valueOf(arrs_poi[5])*78.0/16) > DISTANCE){
				continue;
			}
			//去除无效基站坐标点（误差过大的基站坐标）
