...
public class Algorithm{
	...
	public double Distance(double lat1, double lng1, double lat2,double lng2) { //求两个点之间的距离
	...
	public double Distance(Point p1,Point p2){//两点之间距离
	...
	public double gps2d(double lat_a, double lng_a, double lat_b, double lng_b) { //返回两个坐标点的相对角度
		double d = 0;
	...
	public Point MinDistPtoLine(Point p,Line L){//点到线段上最近的点 精确值(三分法)
	    if(compare(L.p[0],L.p[1]))//如果两个点是同一点
	    	return L.p[0];
	    Point res = new Point();
	...
	public boolean SectorIntersectLine(Line L,Point p,double Ta,sector sec,double downerror,double uperror){//判断弧是否和扇形相交
	...
	public boolean islegalLine(Line L,AnchorPoint p){//判断弧L是否属于定位点P的候选弧集
	...
	public Point getLocationPoint(Point p,double TA,Line L){//返回在弧上的定位点(直接返回中点竟然效果更好。。。。)
